...
//    }
        ...
        /******************************************************************************************
         * This section added by Wes Stephens (wstephens) replacing commented section above 
         * updated: 2012-12-14
         * 
         * This code adds the hierarchy from location level to end to the drop down list for
         * related tables in in crud forms. 
         * 
         * Many hierarchy levels are named generic names or numbers and without the hierarchy 
         * prefacing this, it is impossible to tell which one you are selecting in the drop down
         ******************************************************************************************/
    	...
    	{
            $hierarchy = '';
            //if level in hierarchy below location, display hierarchy in drop down in forms
            if($related_table == 'aisle' || $related_table == 'bay' || $related_table == 'shelf' || $related_table == 'slot' || $related_table == 'sub_slot' || $related_table == 'port' || $related_table == 'sub_port'){
              $this->load->model('inv_model');
                $temp = $this->inv_model->get_hierarchy($related_table, $row->$related_primary_key);
                
                for($i=3;$i<sizeof($temp['list']); $i++){
                    if($i == 3){
                        $hierarchy = $hierarchy . $temp['list'][$i];
                    }else{
                        if($temp['list'][$i] != 'Sub Slot 0'){
                            $hierarchy = $hierarchy . ' ► ' . $temp['list'][$i];
                        }
                    }
                }
                ...
                if($hierarchy != ''){
                    $relation_array[$row->$related_primary_key] = $hierarchy;
                }else{
                    $relation_array[$row->$related_primary_key] = ($row->$field_name_hash < 10 && $row->$field_name_hash > 0?'0'.$row->$field_name_hash:$row->$field_name_hash);	
                }
    ...
    function get_relation_n_n_selection_array($primary_key_value, $field_info)
    {
        ...
        /*Changed by Wes Stephens (wstephens) 12/28/2012
        * replaced line above with if block for when selected field is also primary key - means it is in relation table as foreign key
        * and primary key of selection table so must make selected field unambiguous by prefacing the table_name.field_name
        */
        if($field_info->title_field_selection_table == $field_info->primary_key_alias_to_selection_table){
            $related_field_title = $field_info->selection_table . '.' . $field_info->title_field_selection_table;
        }else{
            $related_field_title = $field_info->title_field_selection_table;
        }
        /********* End Changes *********/
