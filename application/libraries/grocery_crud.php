		...
		if($validation_result->success)
		{
			...
			if($this->callback_insert === null)
			{
				...
				if($this->callback_after_insert !== null)
				{
					...
					{
						return false;
					}else{ //else statement added by Wes Stephens (wstephens)
...
class grocery_CRUD_Layout extends grocery_CRUD_Model_Driver
{
	...
	protected function get_relation_input($field_info,$value)
	{
		...
		$select_title = str_replace('{field_display_as}',$field_info->display_as,$this->l('set_relation_title'));
                //Updated by Wes Stephens (wstephens) - changed width to auto from 300px
		$input = "<select id='field-{$field_info->name}'  name='{$field_info->name}' class='$ajax_or_not_class' data-placeholder='$select_title' style='width:auto'>";
	...
	{	
		...
		else
		{
			...
			//if($field_info_type->name != 'circuit'){
                        ...
                        //}else{
                        //    $input = "<select id='field-{$field_info_type->name}' name='{$field_info_type->name}' class='chosen-select' data-placeholder='".$select_title."'>";
                        //}
...
class grocery_CRUD_States extends grocery_CRUD_Layout
{
	...
	public function getStateInfo()
	{
		...
		switch ($state_code) {
			...
			case 10:
                                //Added by Wes Stephens (wstephens)
                                //update_validation state was not returning primary key as state for getStateInfo
				if($first_parameter !== null)
				{
					$state_info = (object)array('primary_key' => $first_parameter);
				}	
